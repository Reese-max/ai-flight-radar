...
export function configured(base,databaseId,accountId,collectorEnabled){
...
if(process.argv[1]&&path.resolve(process.argv[1])===fileURLToPath(import.meta.url)){
  ...
  const config=configured(base,process.env.CF_RADAR_DATABASE_ID,process.env.CLOUDFLARE_ACCOUNT_ID,process.env.CF_RADAR_COLLECTOR_ENABLED);
